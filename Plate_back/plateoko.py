...
- Na zdjęciu z góry (Top-View) zmierz średnicę naczynia względem sztućca.
...
def analyze_geometry_final(project_id, location, model_name, path_top, path_side):
    ...
    # 1. Dodajemy zdjęcie z GÓRY (Top) - Model oczekuje go jako pierwszego
    try:
    ...
    # 2. Dodajemy zdjęcie z BOKU (Side) - Model oczekuje go jako drugiego
    try:
    ...
    # Konfiguracja (Poprawiony zapis safety settings)
    safety_settings = {
    ...
    try:
        ...
        print("="*45)
        print(f"TYP NACZYNIA:     {geo.get('vessel_type_detected')}")
        print(
            f"REFERENCJA:       {geo.get('reference_object_desc')} ({geo.get('reference_length_mm')} mm)")
        print(f"METODA:           {geo.get('method_used')}")
        print("-" * 45)
        print(f"ŚREDNICA (WYNIK): {geo.get('calculated_diameter_mm')} mm")
        print("-" * 45)
        print(f"OPIS: {geo.get('description')}")
        print("="*45)

        # Opcjonalnie: Wyświetl pełny JSON
...
if __name__ == "__main__":
    analyze_geometry_final(
        PROJECT_ID, LOCATION, MODEL_NAME, IMG_PATH_TOP, IMG_PATH_SIDE)
